...
# I don't know exactly how long the input buffer has to be but in very rare cases with a length
...
.set LOCAL_INPUT_BUFFER_LEN, 2 * ROLLBACK_MAX_FRAME_COUNT
...
.set ODB_ROLLBACK_LOCAL_INPUTS, ODB_ROLLBACK_LOCAL_INPUTS_IDX + 1 # PAD_REPORT_SIZE * LOCAL_INPUT_BUFFER_LEN
.set ODB_ROLLBACK_PREDICTED_INPUTS_READ_IDXS, ODB_ROLLBACK_LOCAL_INPUTS + PAD_REPORT_SIZE * LOCAL_INPUT_BUFFER_LEN # u8
...
.set ODB_ROLLBACK_PREDICTED_INPUTS, ODB_ROLLBACK_PREDICTED_INPUTS_WRITE_IDXS + 1*3 # PAD_REPORT_SIZE * LOCAL_INPUT_BUFFER_LEN
.set ODB_SAVESTATE_IS_PREDICTING, ODB_ROLLBACK_PREDICTED_INPUTS + PAD_REPORT_SIZE * LOCAL_INPUT_BUFFER_LEN * 3 # bool
...
.set RXB_OPNT_INPUTS, RXB_SMALLEST_LATEST_FRAME + 4  # PAD_REPORT_SIZE * ROLLBACK_MAX_FRAME_COUNT * 3
